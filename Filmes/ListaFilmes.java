...
import TelaInicial.InicialTela;
...
public class ListaFilmes extends javax.swing.JFrame implements Interface {
    ...
    public ListaFilmes(InicialTela inicialtela) {
        ...
        //diferencia o tipo de lista criada, T é de todos os filmes,
        //U de usuários e D das dicas de filme.
    ...
    private void criaListaDicaDeFilmes() {//cria a lista de dicas de filmes baseada
        //na categoria dos filmes votados pelo usuario.

        DefaultListModel lista = new DefaultListModel();

        if (controleListaUsuario.usuarioLogado().getFilmesVotados().size() > 1) {
            ...
            Categoria categoria = controleListaUsuario.usuarioLogado().getFilmesVotados().get(0).getCategoria();
            for (Movie filme : controleListaFilme.getFilmes()) {
                if (filme.getCategoria() == categoria) {
                    lista.addElement(filme.getTitulo());
                }
