...
public class UsuarioTela extends javax.swing.JFrame implements Interface {
    ...
    private boolean usuarioVoto(String usuarionome) {
    ...
    private void btnRmvActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRmvActionPerformed
        ...
        if (usuariologado.logado()) {
            String nomeUsuario = JOptionPane.showInputDialog("Digite o Nome do usuario a ser deletado:");
            if (!usuarioVoto(nomeUsuario)) {
                boolean aux = false;
                for (Usuario usuario : controleListaUsuario.getUsuariosCadastrados()) {
                ...
                if (aux) {
                    ...
                    for (Usuario usuario : controleListaUsuario.getUsuariosCadastrados()) {
                        if (usuario.getNome().equals(nomeUsuario)) {
                            controleListaUsuario.getUsuariosCadastrados().remove(usuario);
                            if (usuariologado.getLogin().equals(nomeUsuario)) {
                                login.getIniciaTela().deslogar();
                            }
                            break;
                        }
                ...
                JOptionPane.showMessageDialog(null, "Usuário já voto");
            }
        } else {
            JOptionPane.showMessageDialog(null, "Precisa estar logado para poder votar em um filme.");
            Login laogin = new Login(login.getIniciaTela());
